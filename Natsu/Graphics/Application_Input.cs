...
public partial class Application {
    ...
    protected static List<Element> ConditionalElementTree(Element cur, Func<Element, bool> condition = null) {
        if (cur == null) {
        ...
        if (cur.Children.Count > 0) {
            lock (cur.Children) {
                for (int i = cur.Children.Count - 1; i >= 0; i--) {
                    if (cur.Children[i] is CachedElement) {
                        continue;
                    }
        ...
        if (condition == null || condition(cur)) {
            Console.WriteLine(cur.GetType().Name + " -> " + cur.Name + " -> " + (condition == null ? "null" : condition.Method.Name) + " -> " + (condition == null ? "null" : condition(cur).ToString()));
            elements.Add(cur);
        }
    ...
    public void ConstructInputLists() {
        PositionalInputList = ConditionalElementTree(Root, (Element x) => x is InputElement && x is not GlobalInputElement).Cast<InputElement>().ToList();
        NonPositionalInputList = ConditionalElementTree(Root, (Element x) => x is GlobalInputElement).Cast<GlobalInputElement>().ToList();
    }
    ...
    public bool MouseDown(MouseButton button, Vector2 position) {
        ...
        if (!_mouseDownCache.ContainsKey(button))
            _mouseDownCache.Add(button, new());
    ...
    public void MouseMove(Vector2 position) {
        ...
        foreach (InputElement elm in _mouseEnterCache.Keys.ToList()) {
            if (!elms.Contains(elm)) {
                if (_mouseEnterCache[elm].HasFlag(MouseEnterCacheState.Over)) {
